...
Скачайте плагин через магазин плагинов Wordpress, установите его и активируйте
...
Далее перейдите в админ-панель cms Wordpress и активируйте плагин "Зооспас - проект для хакатона"

### Из архива

Скачайте архив с плагином по [ссылке](https://github.com/xakplant/zoospas/archive/master.zip). Перейдите в админ панель
cms Wordpress на стриницу "Плагины" > "Добавить новый". Нажмите на кнопку "Загрузить плагин" рядом с заголовком
"Добавить плагины". В появишиеся окно вставьте архив с плагином. Далее активирутей плагин.
...
В карточке животного есть как типичные для каждой записи wordpress'а свойства так и специфические. У каждого животного 
...
Для стилизации фильтра есть несколько хуков. Схематично представить послеврвательность хуков можно так:
...
Хук принимает переменную $key значение которой равно название meta-поля "meta key". Его использование не обязательно, но
...
распечатана внутри кнопки, как атрибут. Пример кода
...
В результатах фильтра выводиться список животных. Схему хуков печати карточки животных можно представить так:
...
zs_card_wraper_start печатает открывающиё тэг оболочки карточки животного. Пример кода:
...
zs_admin_edit_post печатает ссылку на страницу редактирование (видно только от администратора). Пример кода:
...
```CSS
.zs_card{} /* Стиль обёртки карточки животнго в фильтре */
...
На странице карточки животного можно вывести данные животного данные шорткодом: 

```PHP
[zs_print_pet_card]
```

Хотя это быстро и удобно, лучшей практикой будет сделать специальный шаблон для типа записи zs_pets. Для этого создайте 
в директории вышей темы файл с имемеи single-zs_pets.php и напишите свой код. Пример кода:
...
условно различаются на "левую" и "правую". Шорткоы соответственно:
...
Их ститистическое отображение будет полностью зависеть от стилей оформления ссылок в теме и настроек в админ-пенеле.

### Форма обратной связи

В плагине предусмотрена "легковесная" форма обратной связи с различными типами отображениея, благодаря которому 
...
JavaScript выводит на страницу HTML, где находиться шорткод. Данный вариант вам будет полезен в случае если вам нужно
...
Выводит ссылку на страницу. Урл задаётся в поле "Ссылка" раздела настроек "Настройки формы обратной связи".

#### Форма

При данном типе на страницу выводиться форма обратной связи. Для формы вы можете дополнительно задать css-класс.

#### Модальное окно

Выводит на страницу кнопку, которая вызывает сплывающее окно с формой обратной связи. 
